...
class IntelligentProductMatcher:
    ...
    def calculate_match_score(self, features1: Dict, features2: Dict) -> float:
        ...
        elif not features1['quantity'] and not features2['quantity']:
            score += 0.2  # Both have no quantity specified
        
        # Brand should match (if present)
        if features1['brand'] and features2['brand']:
            if features1['brand'] == features2['brand']:
                score += 0.3
            else:
                return 0.0  # Different brands = different products
        elif not features1['brand'] and not features2['brand']:
            score += 0.15  # Both have no brand
        
        # Flavors should match
        if features1['flavors'] and features2['flavors']:
            common_flavors = set(features1['flavors']) & set(features2['flavors'])
            if common_flavors:
                score += 0.2
            else:
                return 0.0  # Different flavors = different products
        elif not features1['flavors'] and not features2['flavors']:
            score += 0.1  # Both have no flavors
        
        # Main words should have significant overlap
        if features1['main_words'] and features2['main_words']:
            words1 = set(features1['main_words'])
            words2 = set(features2['main_words'])
            if words1 and words2:
                overlap = len(words1 & words2) / max(len(words1), len(words2))
                if overlap > 0.3:
                    score += 0.1
                else:
                    return 0.0  # Not enough word overlap
        ...
        for i, product1 in enumerate(all_products):
            ...
            for j, product2 in enumerate(all_products[i+1:], start=i+1):
                ...
                if score > 0.5:  # Threshold for matching
                    ...
                    matched.add(j)
                    match_count += 1
                    print(f"✓ MATCH (score: {score:.3f}): {product1['store']:10} '{product1['name'][:40]:40}' <-> {product2['store']:10} '{product2['name'][:40]:40}'")
