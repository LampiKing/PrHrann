...
class IntelligentProductMatcher:
    def __init__(self, config_file='store_config.json'):
        ...
        # Common product descriptors to ignore (too generic for matching)
        self.descriptors = {
            # Generične besede za živila
            'seme', 'semena', 'semen', 'zrno', 'zrna', 'zrnje',
            'moka', 'moke', 'kosmiči', 'kosmičev', 'ovseni', 'ovsena',
            'kruh', 'kruha', 'pecivo', 'peciva', 'test', 'testenine',
            # Pridevniki
        ...
        # KRITIČNE besede ki definirajo TIP izdelka - morajo se ujemati!
        self.product_types = {
            # Semena - različni tipi
            'laneno', 'lanena', 'sončnično', 'sončnična', 'sezamovo', 'sezamova',
            'bučno', 'bučna', 'chia', 'konopljino', 'konopljina', 'makovo', 'makova',
            # Olja - različni tipi (POMEMBNO!)
            'oljčno', 'olivno', 'sončnično', 'repično', 'bučno', 'sezamovo',
            'kokosovo', 'palmovo', 'arašidovo', 'orehovo', 'laneno',
            # Oreščki - različni tipi
            'indijski', 'lešniki', 'mandlji', 'orehi', 'arašidi', 'pistacije',
            'pekan', 'makadamija', 'brazilski',
            # Sadje - različni tipi
            'marelice', 'slive', 'fige', 'rozine', 'brusnice', 'jagode',
            'borovnice', 'maline', 'jabolka', 'hruške', 'breskve', 'ananas',
            'mango', 'banana', 'pomaranče', 'limone', 'grenivke', 'kivi',
            # Zelenjava
            'čebula', 'česen', 'krompir', 'korenje', 'paradižnik', 'paprika',
            'kumare', 'bučke', 'zelje', 'solata', 'špinača', 'brokoli',
            # Meso
            'piščanec', 'puran', 'govedina', 'svinjina', 'teletina', 'jagnjetina',
            # Mlečni izdelki
            'mleko', 'jogurt', 'skuta', 'sir', 'maslo', 'smetana', 'kefir',
            # Pijače
            'sok', 'nektar', 'voda', 'pivo', 'vino', 'kava', 'čaj',
            # Okusi čokolad
            'mlečna', 'temna', 'bela', 'grenka',
        }
    ...
    def extract_key_features(self, name: str) -> Dict:
        ...
        # Extract product type (KRITIČNO - mora se ujemati!)
        product_type = None
    ...
    def calculate_match_score(self, features1: Dict, features2: Dict) -> float:
        ...
        Returns 0-1 score
        Stricter matching - requires product type match + brand/word overlap
        """
        score = 0.0
        has_brand_match = False
        has_word_match = False

        # KRITIČNO: Če imata oba izdelka product_type, se MORATA ujemati!
        pt1 = features1.get('product_type')
        pt2 = features2.get('product_type')
        if pt1 and pt2:
            if pt1 != pt2:
                return 0.0  # Različna tipa izdelka = NI ISTI IZDELEK!
